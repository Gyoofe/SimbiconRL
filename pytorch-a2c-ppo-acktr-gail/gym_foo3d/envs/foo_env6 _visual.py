...
class FooEnv6(env_base.FooEnvBase):
    def init_sim(self,cDirection,render):
        ...
        self.leftFoot = 0
        self.FXAnorm = [1,0,0]
        self.l_foot_pos = cMat.Matrix.col(self.l_foot.world_transform(),3)
        self.r_foot_pos = cMat.Matrix.col(self.r_foot.world_transform(),3)
    ...
    def StartMinimum(self):
        self.desiredStepDuration = 0.1
        self.desiredStepLength = 0.2/3
        self.desiredMaximumSwingfootHeight = 0.1/4

    def StartMedian(self):
        self.desiredStepDuration = 0.25
        self.desiredStepLength = 0.5/3+0.1
        self.desiredMaximumSwingfootHeight = 0.075 + self.desiredStepDuration/4
    ...
    def do_simulation(self, action):
        ...
        while(self.previousState is self.controller.mCurrentStateMachine.mCurrentState.mName
            ...
            self.r_foot_pos = r_foot_pos
            self.l_foot_pos = l_foot_pos
            ...
            cFXA = self.getCOMFrameXAxis()
            currentFrameXAxisN = np.linalg.norm(cFXA)
            self.FXAnorm = cFXA/currentFrameXAxisN
            ##Step이전 State에 따라서 정해짐
            ##rFoot 올릴때(rFoot Contact가 일어났다가 떨어짐)
            if self.previousState is "0":
                #rightFoot = np.dot(self.last_Rcontact_r_foot_pos - pos_after, self.currentFrameXAxis)/currentFrameXAxisN
                #leftFoot = np.dot(self.last_Rcontact_l_foot_pos - pos_after, self.currentFrameXAxis)/currentFrameXAxisN
                rightFoot = np.dot(r_foot_pos - pos_after, cFXA)/currentFrameXAxisN
                leftFoot = np.dot(l_foot_pos - pos_after, cFXA)/currentFrameXAxisN
                StepLengthPenalty = np.abs(rightFoot - leftFoot - self.desiredStepLength)
                self.cStepLength = rightFoot - leftFoot
                self.endFoot = pos_after + leftFoot*self.FXAnorm
                self.endFoot[1] = -0.95
